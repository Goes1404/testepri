-- ============================================================================
-- DELTA DE PRODUÇÃO — Portal do Aluno (projeto cryeesunxnfgkshvafbo)
-- Gerado a partir do diagnóstico de 07/07/2026: o banco já possui o schema base
-- e os seeds (universities, scholarships, deadlines, events, cut_score_history).
-- Este arquivo cria APENAS o que falta. Idempotente — seguro reexecutar.
--
...
-- ═══════════════ 2. NPS (nps_responses, 1 resposta por ciclo) ═══════════════
create table if not exists public.nps_responses (
    id uuid default gen_random_uuid() primary key,
    user_id uuid references public.users(id) on delete set null,
    score integer not null check (score >= 0 and score <= 10),
    comentario text,
    ciclo text not null default 'beta-2026',
    created_at timestamp with time zone default timezone('utc'::text, now()) not null
);

alter table public.nps_responses enable row level security;

drop policy if exists "Users can insert own nps response" on public.nps_responses;
create policy "Users can insert own nps response" on public.nps_responses
    for insert with check (auth.uid() = user_id);

drop policy if exists "Users can read own nps response" on public.nps_responses;
...
-- ═══════════════ 3. QUESTÕES DO SIMULADO (quiz_questions) ═══════════════
...
-- ═══════════════ 5. ÍNDICES DE PERFORMANCE (Hub B, < 100ms) ═══════════════
...
-- ═══════════════ 7. updated_at AUTOMÁTICO (boas práticas) ═══════════════
...
-- ═══════════════ 8. VERIFICAÇÃO FINAL ═══════════════
-- Deve retornar 22 tabelas, incluindo audit_logs, nps_responses, quiz_questions e push_tokens
