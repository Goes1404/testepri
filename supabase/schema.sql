...
-- Renewals table (semestral renewal tracking for scholarship holders)
create table public.renewals (
    id uuid default gen_random_uuid() primary key,
    application_id uuid references public.applications(id) on delete cascade not null,
    semestre text not null, -- e.g., "2025/1"
    ira numeric not null, -- Índice de Rendimento Acadêmico (0-10)
    frequencia numeric not null, -- attendance percentage 0-100
    situacao text default 'pendente'::text not null, -- pendente, aprovado, reprovado
    observacao text,
    created_at timestamp with time zone default timezone('utc'::text, now()) not null
);
...
-- public.renewals Policies
create policy "Users can view their own renewals." on public.renewals
    for select using (
        exists (
            select 1 from public.applications
            where public.applications.id = application_id and public.applications.user_id = auth.uid()
...
create policy "Users can insert their own renewals." on public.renewals
    for insert with check (
        exists (
            select 1 from public.applications
            where public.applications.id = application_id and public.applications.user_id = auth.uid()
